...
import { uploadFile } from "../Dynamic Form/Forms";
import SignaturePad from "../Dynamic Form/Signaturepad";
...
export default function FormField({
  ...
  const inputClasses = clsx("input w-full", error && "border-danger/60 focus:border-danger/60");
  ...
  async function handleFiles(fileList: FileList | null) {
    ...
    try {
      ...
      // Upload failed — the field simply keeps its prior value; the technician can retry.
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    <label className="block">
      ...
      {field.type === "select" && (
        <select className={inputClasses} value={(value as string) ?? ""} onChange={(e) => onChange(e.target.value)}>
      ...
      {field.type === "boolean" && (
        <button
          ...
          className={clsx(
            ...
            value ? "border-teal-dim/40 bg-teal-soft text-teal" : "border-border bg-surface3 text-faint"
          )}
        >
        ...
        <div>
          <div className="mb-2 flex flex-wrap gap-1.5">
            {((value as UploadedFile[]) ?? []).map((f) => (
              ...
              >
                {f.name}
                <button type="button" onClick={() => removeFile(f.key)} className="text-faint hover:text-danger">
                  <X size={11} />
                </button>
              </span>
            ))}
          ...
          >
            ...
            {uploading ? `Uploading… ${uploadPct}%` : field.type === "image" ? "Add photo" : "Add file"}
            <input
              type="file"
        ...
        <div>
          {value && (
            <div className="mb-2 flex items-center gap-1.5 font-mono text-[11px] text-lo">
              ...
              {(value as LocationValue).lat.toFixed(5)}, {(value as LocationValue).lng.toFixed(5)}
            </div>
          )}
          <button
            type="button"
      ...
      {field.type === "signature" && <SignaturePad onChange={(dataUrl) => onChange(dataUrl)} />}

      {field.helpText && !error && <p className="mt-1 text-[11px] text-faint">{field.helpText}</p>}
